...
3. Use Pillow 12.3.0 with its LittleCMS-backed `ImageCms` path for the bounded
   deterministic Recovery 2E execution path. Export is limited to 64 MiB source
   and output objects, 16 megapixels, a 12,000 pixel edge, 768 MiB process RSS,
   60 seconds and one job per worker process. The Studio preparation preview has
   a separate 100 megapixel admission ceiling because it produces a bounded
   2,048 pixel workspace proxy; it is not the enhancement comparison path. Check
   cancellation and process budgets between expensive stages.
