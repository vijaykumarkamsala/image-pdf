...
export const MIGRATION_VERSION = "0001_recovery_2a_product_kernel";
...
export async function runMigrations(pool: Pool): Promise<void> {
  ...
  if (existing.rows[0]?.present) {
    const applied = await pool.query("SELECT 1 FROM schema_migrations WHERE version = $1", [MIGRATION_VERSION]);
    if (applied.rowCount) return;
  }

  const here = dirname(fileURLToPath(import.meta.url));
  const migration = resolve(here, "..", "..", "..", "migrations", `${MIGRATION_VERSION}.sql`);
