...
@Module({
  imports: [KernelModule, IntakeModule, JobsModule],
  controllers: [ExperienceController],
  providers: [
    {
      provide: EXPERIENCE_REPOSITORY,
      async useFactory(product: ProductKernelRepository, jobs: DurableJobRepository, intake: IntakeRepository) {
        ...
        return new MemoryExperienceRepository(product, jobs, intake);
      },
      inject: [PRODUCT_REPOSITORY, DURABLE_JOB_REPOSITORY, INTAKE_REPOSITORY],
    },
    ExperienceService,
  ],
