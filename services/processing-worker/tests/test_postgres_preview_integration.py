...
def test_real_postgres_preview_job_records_provenance_audit_and_zero_charge(tmp_path: Path) -> None:
    ...
    try:
        ...
        cursor.execute(
            ...
            "media_type,byte_size,created_at) VALUES(%s,%s,%s,%s,'image/png',%s,now())",
        ...
        cursor.execute(
            "SELECT zoom_level,width,height,source_sha256,authoritative "
        ...
        assert all(
            row[1] <= 2048 and row[2] <= 2048 and row[3] == digest and row[4] is False
