...
function NotificationCenter({ workspaceId, refresh }: { workspaceId: string; refresh: number }) {
  ...
  const load = useCallback(() => {
    api.notifications(workspaceId, undefined, 12).then((response) => {
      setNotifications((current) => {
        if (!hasOlderPages.current) return response.notifications;
        const fresh = new Set(response.notifications.map((item) => item.notification_id));
        return [...response.notifications, ...current.filter((item) => !fresh.has(item.notification_id))];
      });
      if (!hasOlderPages.current) setCursor(response.next_cursor ?? null);
  ...
  async function more() {
    ...
    setLoadingMore(true);
    try {
      const response = await api.notifications(workspaceId, cursor, 12);
      setNotifications((current) => {
        const present = new Set(current.map((item) => item.notification_id));
        return [...current, ...response.notifications.filter((item) => !present.has(item.notification_id))];
      });
      hasOlderPages.current = true;
      setCursor(response.next_cursor ?? null);
      setUnread(response.unread_count);
      setAnnouncement(`${response.notifications.length} older ${response.notifications.length === 1 ? "notification" : "notifications"} loaded.`);
      setError(false);
    } catch {
      setError(true);
    } finally {
  ...
  return <div className="notification-center"><Popover label={unread ? `${unread} unread notifications` : "Notifications"} trigger={<span className="notification-trigger"><Bell aria-hidden="true" />{unread > 0 && <span>{unread > 99 ? "99+" : unread}</span>}</span>}>
    <div className="notification-popover"><div className="notification-heading"><div><strong>Notifications</strong><span>{unread ? `${unread} unread` : "You're up to date"}</span></div>{unread > 0 && <Button tone="quiet" onClick={() => void markAll()}>Mark all read</Button>}</div>
      {error ? <InlineNotice tone="error" title="Notifications unavailable">Refresh and try again.</InlineNotice>
        : notifications.length === 0 ? <StatePanel kind="empty" title="No notifications" message="File and job updates will appear here." />
          : <><div className="notification-list">{notifications.map((item) => <button type="button" className={item.read_at ? "is-read" : "is-unread"} key={item.notification_id} onClick={() => void markRead(item)}><span className="notification-dot" /><span><strong>{item.title}</strong><small>{item.message}</small></span></button>)}</div>
            {(cursor || announcement) && <div className="notification-pagination">{cursor && <Button tone="quiet" disabled={loadingMore} onClick={() => void more()}>{loadingMore ? "Loading" : "Load more"}</Button>}<span role="status" aria-live="polite">{announcement}</span></div>}</>}
    </div>
  </Popover></div>;
}
...
export function SignedWorkspaceHome({ workspaceId, actorName, onUpload, refresh }: {
  ...
  return <main className="page home-page" data-testid="workspace-home">
    ...
    <section className="home-section operational-section" aria-labelledby="attention-heading"><div className="section-heading"><div><h2 id="attention-heading">Needs attention</h2><p>Only work with a current action you can take appears here.</p></div></div>{home.attention.length === 0 ? <div className="quiet-state"><CheckCircle2 aria-hidden="true" /><span>Nothing needs your attention.</span></div> : <div className="attention-list">{home.attention.map((item) => <button type="button" key={`${item.kind}-${item.resource_id}`} onClick={() => navigate(item.path)}><AlertTriangle aria-hidden="true" /><span><strong>{item.title}</strong><small>{item.message}</small></span></button>)}</div>}</section>
...
export function JobsPage() {
  ...
  const panel = (() => {
    ...
    return <div className="job-list">{deepLinkLoading && <StatePanel kind="loading" title="Opening job timeline" message="Retrieving the permitted job and its ordered events." />}{deepLinkError && <InlineNotice tone="error" title="Job unavailable">{deepLinkError}</InlineNotice>}{visibleJobs.map((item) => {
      ...
      const preview = item.kind === "preview_generation";
      return <article className="job-card" key={item.job_id} data-job-id={item.job_id}><header><span className={`job-state-icon state-${item.state}`}>{active ? <LoaderCircle aria-hidden="true" /> : item.state === "succeeded" ? <CheckCircle2 aria-hidden="true" /> : item.state === "cancelled" ? <XCircle aria-hidden="true" /> : <AlertTriangle aria-hidden="true" />}</span><div><h2>{preview ? "Studio preview" : "File intake check"}</h2><p>{preview ? "Safe editing proxy" : "1 file"}, {item.progress_percent}% complete</p></div><Badge tone={stateTone(item.state)}>{stateLabel(item.state)}</Badge></header>{item.failure && <InlineNotice tone={item.failure.retryable ? "warning" : "error"} title={item.failure.retryable ? "Retry available" : "Job did not finish"}>{item.failure.message}</InlineNotice>}<div className="job-actions"><Button tone="quiet" onClick={() => void timeline(item.job_id)}><History aria-hidden="true" />Timeline</Button>{active && item.state !== "cancel_requested" && <Button tone="danger" onClick={() => void cancel(item)}>Cancel</Button>}{item.state === "failed" && item.failure?.retryable && <Button onClick={() => void retry(item)}><RotateCcw aria-hidden="true" />Retry</Button>}</div>{selectedJob === item.job_id && <section className="job-timeline" aria-label="Ordered job timeline">{itemEvents ? itemEvents.map((event) => <div key={event.job_event_id}><span /><div><strong>{event.event_kind.replaceAll(".", " ")}</strong><small>{stateLabel(event.state)}, {event.progress_percent}%</small></div></div>) : <StatePanel kind="loading" title="Loading timeline" message="Retrieving ordered job events." />}<details><summary>Advanced reference details</summary><dl><div><dt>Job</dt><dd>{item.job_id}</dd></div>{item.upload_session_id && <div><dt>Upload</dt><dd>{item.upload_session_id}</dd></div>}{item.document_id && <div><dt>Document</dt><dd>{item.document_id}</dd></div>}{itemEvents?.at(-1) && <div><dt>Trace</dt><dd>{itemEvents.at(-1)!.trace_id}</dd></div>}</dl></details></section>}</article>;
    })}{cursor && <Button onClick={() => void more()}>Load more</Button>}</div>;
  })();

  return <main className="page jobs-page" data-testid="jobs-page"><section className="page-heading"><div><p className="eyebrow">Workspace</p><h1>Jobs</h1><p>Follow file checks, retries and completion without keeping this page open.</p></div><IconButton label="Refresh Jobs" onClick={() => setGeneration((current) => current + 1)}><RefreshCw aria-hidden="true" /></IconButton></section><Tabs label="Job views" selected={view} onSelect={(id) => {
