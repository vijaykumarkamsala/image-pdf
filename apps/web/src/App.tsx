  ...
  return <div className="app-shell">
    ...
    <div className="app-main">
      ...
      <Routes>
        ...
        <Route path=":workspaceId/files" element={<FilesPage defaultFilesName={context.default_files.name ?? "Default Files"} refresh={fileRefresh} onUpload={() => setUploadOpen(true)} canCreateBatch={context.effective_permissions.some((permission) => permission.permission === "batch.create" && permission.allowed)} />} />
...
function ProjectsPage() {
  ...
  return <main className="page" data-testid="projects-page">
    ...
    {projects === null || documents === null ? <StatePanel kind="loading" title="Loading projects" message="Retrieving work you can access." /> : projects.length === 0 ? <StatePanel kind="empty" title="No projects yet" message="Create a project when you want files grouped around one piece of work." action={{ label: "New project", onClick: () => setCreating(true) }} /> : <div className="project-grid">{projects.map((project) => {
      ...
      return <article className="project-card project-card-with-contents" key={project.project_id}><span className="project-icon"><BriefcaseBusiness aria-hidden="true" /></span><div><h2>{project.name}</h2><p>{project.parent_project_id ? "Subproject" : "Project"}</p></div><span className="status-dot">Active</span><div className="project-contents"><strong>Native documents</strong>{contents.length ? <div role="list" aria-label={`${project.name} native documents`}>{contents.map((document) => <NativeDocumentCard key={document.document_id} document={document} projects={projects} workspaceId={workspaceId} onMoved={(moved) => setDocuments((current) => current?.map((item) => item.document_id === moved.document_id ? moved : item) ?? null)} />)}</div> : <span>No native documents</span>}</div></article>;
    })}</div>}
...
function FilesPage({ defaultFilesName, refresh, onUpload, canCreateBatch }: { defaultFilesName: string; refresh: number; onUpload: () => void; canCreateBatch: boolean }) {
  const { workspaceId = "" } = useParams();
  const navigate = useNavigate();
